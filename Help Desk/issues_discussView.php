        ...
        if ($allowed) {
            ...
            if ($isTechnician && !$isPersonsIssue && $settingGateway->getSettingByScope('Help Desk', 'techNotes')) {
                ...
                $row = $form->addRow();
                    ...
                    $col->addButton(__('Add Technician Note'))->setIcon('solid', 'plus')->addClass('float-right')->setAttribute('@click', 'comments = !comments');
            ...
            if ($issue['status'] == 'Pending' && ($isRelated || $hasFullAccess)) {
                $col->addButton(__('Add Comment'))->setIcon('solid', 'plus')->addClass('float-right')->setAttribute('@click', 'comments = !comments');
